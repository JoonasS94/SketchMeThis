...
public class DrawManager3 : MonoBehaviour
{
    ...
    [SerializeField] private Line3 _linePrefab;
    public const float RESOLUTION = .005f;
    // Remember to change LineX depending on scene
    private Line3 _currentLine;
    ...
    private float ratioNumber = 1.219512f;
    ...
    void Start()
    {
        ...
        PointCountObject = GameObject.Find("PointCounter3");
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && _pauseGameAreaCollider.OverlapPoint(mousePos) && PointCountObject.GetComponent<PointCount3>().canDraw == true)
        {
        ...
        if (Input.GetMouseButtonDown(0) && _nextDrawingAreaCollider.OverlapPoint(mousePos))
        {
            ...
            FindObjectOfType<AudioManager>().Play("SFX_ButtonPress1");
            SceneManager.LoadScene("Draw4");
        }

        // Remember to change <PointCountX> depending on scene
        // Stop drawing pressed while game is active
        if (Input.GetMouseButtonDown(0) && _stopDrawingAreaCollider.OverlapPoint(mousePos) && PointCountObject.GetComponent<PointCount3>().canDraw == true && PointCountObject.GetComponent<PointCount3>().gamePaused == false)
        {
            // Remember to change <PointCountX> depending on scene
            FindObjectOfType<AudioManager>().Play("SFX_ButtonPress2");
            PointCountObject.GetComponent<PointCount3>().DrawingDistanceInTotal = 999;
        }


        // Remember to change <PointCountX> depending on scene
        // Drawing
        if (_drawAreaCollider.OverlapPoint(mousePos) && PointCountObject.GetComponent<PointCount3>().canDraw == true && PointCountObject.GetComponent<PointCount3>().gamePaused == false)
        {
        ...
        if (PointCountObject.GetComponent<PointCount3>().canDraw == false && PointCountObject.GetComponent<PointCount3>().StopChecking == true && CompareResultsStarted == false)
        {
    ...
    IEnumerator PauseGame()
    {
        ...
        // Remember to change <PointCountX> depending on scene
        PointCountObject.GetComponent<PointCount3>().gamePaused = true;
    ...
    IEnumerator DisablePauseGame()
    {
        ...
        // Remember to change <PointCountX> depending on scene
        PointCountObject.GetComponent<PointCount3>().gamePaused = false;
        yield return new WaitForSeconds(0.25f);
    ...
    IEnumerator StartGamePlay()
    {
        ...
        PointCountObject.GetComponent<PointCount3>().canDraw = true;
    }

    IEnumerator CompareResults()
    {
        FindObjectOfType<AudioManager>().Play("SFX_UI_TEXT_06");
        ...
        // Remember to change <PointCountX> depending on scene
        DrawingResult = (PointCountObject.GetComponent<PointCount3>().PointTotalCounter * ratioNumber);
        ...
        GameObject.Find("Score3").GetComponent<DrawScore3>().draw3Score = RoundingToInt;
        ...
        FindObjectOfType<AudioManager>().Play("SFX_UI_TEXT_01");
